...
export default function AdminMarketPrices() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    fetchReferencePrices(deliveryDate, SOURCE)
      .then((data) => {
        if (cancelled) return;
      ...
      .catch((err) => setError(err instanceof ApiError ? err.message : "Could not load local market prices."))
      .finally(() => {
  ...
  async function saveField(productId: number, raw: string) {
    ...
    try {
      await setReferencePrice(productId, deliveryDate, value, SOURCE);
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-2xl shadow-card border border-sage-100 p-4">
        ...
        {deliveryDate && (
          <p className="text-xs text-crate-800/40 mt-3">
            Local market prices entered here for {formatDate(deliveryDate)} are a reference for Admin only —
            separate from Keells Prices, and not shown to suppliers or used in any price calculation.
          </p>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-card border border-sage-100 overflow-hidden">
        {loading ? (
          ...
          <EmptyState icon={<IconBranches width={20} height={20} />} title="No products match your filters" />
        ) : (
